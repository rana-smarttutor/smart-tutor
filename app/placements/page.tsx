...
export default async function PlacementsPage() {
  ...
  const toppers = data.placedStudents.slice(0, 3);
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 bg-blue-600/5 dark:bg-blue-900/10 -z-10" />
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto mb-20">
            {[
              ...
              <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl shadow-blue-500/5 border border-white dark:border-slate-800 text-center">
          ...
          {/* Feature Toppers Display - "The New System" */}
          <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {toppers.map((topper, i) => (
              <RevealOnScroll key={topper.id} delayMs={i * 100}>
                <div className="relative group overflow-hidden rounded-[3rem] bg-white dark:bg-slate-900 shadow-2xl border border-slate-100 dark:border-slate-800 h-full">
                  ...
                  <div className="p-8 text-center bg-white dark:bg-slate-900 relative">
                     <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-24 h-24 bg-blue-600 rounded-full flex items-center justify-center border-8 border-white dark:border-slate-900 shadow-xl">
                        <span className="text-white text-2xl font-black">{topper.rank === "1st" ? "🏆" : "⭐"}</span>
                     </div>
                     <div className="pt-8">
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-1">{topper.name}</h3>
                        <p className="text-blue-600 font-bold text-sm mb-4">{topper.rank === "1st" ? "All India Rank 1" : "Top Percentiler"}</p>
                        <p className="text-slate-500 text-xs italic">"Smart Tutors' mentoring was the key to my success in {topper.examName}."</p>
                     </div>
                  </div>
                </div>
              </RevealOnScroll>
      ...
      <div className="py-20 border-t border-slate-200 dark:border-slate-800">
        <div className="container mx-auto px-4">
            <div className="text-center mb-10">
                <h2 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white mb-4">Complete Results Gallery</h2>
