...
export default function QuizArenaClient({
      ...
      try {
        ...
        if (data.draft) {
          ...
          try {
            ...
            // If browser storage is unavailable, keep the
            // existing reminder behaviour as a fallback.
          }
  ...
  function resumePreviousQuiz() {
    ...
    // Resume the actual saved quiz, including answers and time.
    setStep("quiz");
  }
  ...
  return (
    <main className="relative min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 px-5 py-8 text-white">
      ...
      >
        ...
        {step === "level" && (
          <section>
            {unfinishedDraft && !showResumeChoice && (
              <div className="mb-8 rounded-3xl border border-cyan-300/30 bg-cyan-400/10 p-5 sm:p-6">
        ...
        {(step === "exam" || step === "ssc") && (
          <section>
            <Header
              title={
                step === "ssc"
                  ? "Choose your SSC examination"
                  : "Choose your course or exam"
              }
              subtitle={
                step === "ssc"
                  ? "Select SSC CGL, SSC CPO, SSC CHSL or SSC GD."
                  : "Pick the exact board, stream or entrance exam you want to practise."
              }
            />
