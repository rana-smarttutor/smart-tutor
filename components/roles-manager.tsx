...
export function RolesManager({ managedUsers }: Props) {
  ...
  function getAssignedUsers(roleId: string) {
    ...
    return assigned.map((a) => managedUsers.find((u) => u.id === a.userId)).filter(Boolean);
  }
  ...
  async function handleDeleteRole(id: string, name: string) {
    if (!confirm(`Delete role '${name}'? Staff with this role will lose these permissions.`)) return;
    try {
      const res = await fetch(`/api/admin/roles/${id}`, {
        method: "DELETE",
        credentials: "same-origin",
  ...
  // Module checkbox toggle for create form
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {role.description && (
                                <div className="text-[11px] text-[var(--color-muted)]">{role.description}</div>
                              )}
                            </div>
                          </div>
          ...
          <div className="surface rounded-[2rem] p-6">
            ...
            <form onSubmit={handleCreateRole} className="space-y-4">
              ...
              <div>
                ...
                <div className="max-h-48 overflow-y-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] p-3">
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                    {AVAILABLE_MODULES.map((mod) => (
                      <label
                        key={mod.id}
                        className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-medium text-[var(--color-body)] hover:bg-[var(--color-primary)]/5"
                      >
                        <input
                          type="checkbox"
                          checked={newRole.modules.includes(mod.id)}
                          onChange={() => toggleModule(mod.id, newRole.modules, (v) => setNewRole({ ...newRole, modules: v }))}
                ...
                {newRole.modules.length > 0 && (
                  <p className="mt-1 text-xs text-[var(--color-muted)]">
                    {newRole.modules.length} module{newRole.modules.length !== 1 ? 's' : ''} selected
                  </p>
                )}
              </div>
              <button
              ...
              <tbody>
                {managedUsers.map((user) => {
                  ...
                  const availableRoles = roles.filter(
                    (r) => !userAssignments.some((a) => a.roleId === r.id)
                  );
                  return (
                    <tr key={user.id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-background-strong)]/50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1.5" id={`badges-${user.id}`}>
                          {userAssignments.length === 0 ? (
                            ...
                            userAssignments.map((a) => (
                              <span key={a.id} className="inline-flex items-center gap-1 rounded-md bg-[#EEF2FF] px-2 py-0.5 text-[11px] font-bold text-[#4338CA]">
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleRemoveRole(user.id, a.roleId)}
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                    {AVAILABLE_MODULES.map((mod) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={editMod.includes(mod.id)}
                          onChange={() => toggleModule(mod.id, editMod, setEditMod)}
                          className="h-4 w-4 rounded border-gray-300 text-[var(--color-primary)] focus:ring-[var(--color-primary)]"
                        />
                        {mod.label}
                      </label>
