...
function useAutoAlert(ms = 3000) {
  const [alert, setAlert] = useState<{ type: AlertType; msg: string } | null>(
    null,
  );
  const timer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const show = useCallback(
    (type: AlertType, msg: string) => {
...
export function DashboardProfileSettings({
  ...
  const [subjects, setSubjects] = useState<string[]>(
    Array.isArray(p?.subjects) ? p.subjects : [],
  );
  ...
  const [course, setCourse] = useState(
    p?.courseWantedTitle ?? p?.courseWanted ?? "",
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (addressLine1 !== (p?.addressLine1 ?? ""))
        body.addressLine1 = addressLine1;
      if (addressLine2 !== (p?.addressLine2 ?? ""))
      ...
      if (profilePhoto !== (p?.profilePhoto ?? ""))
        body.profilePhoto = profilePhoto;

      if (isEducator) {
        if (qualification !== (p?.qualification ?? ""))
        ...
        if (JSON.stringify(subjects) !== JSON.stringify(origSubjects))
          body.subjects = subjects;
      }

      if (isStudent) {
        if (parentName !== (p?.parentName ?? "")) body.parentName = parentName;
        if (parentEmail !== (p?.parentEmail ?? ""))
          body.parentEmail = parentEmail;
        if (parentMobile !== (p?.parentMobile ?? ""))
          body.parentMobile = parentMobile;
        if (course !== (p?.courseWantedTitle ?? p?.courseWanted ?? ""))
          body.courseWantedTitle = course;
        if (studentType !== (p?.studentType ?? ""))
          body.studentType = studentType;
        if (latestQualification !== (p?.latestQualification ?? ""))
          body.latestQualification = latestQualification;
        if (latestAcademicScore !== (p?.latestAcademicScore ?? ""))
          body.latestAcademicScore = latestAcademicScore;
        if (
          JSON.stringify(strongSubjects) !==
          JSON.stringify(p?.strongSubjects ?? [])
        )
          body.strongSubjects = strongSubjects;
        if (
          JSON.stringify(weakSubjects) !== JSON.stringify(p?.weakSubjects ?? [])
        )
          body.weakSubjects = weakSubjects;
      }
          ...
          <div className="relative h-28 w-28 shrink-0">
            {profilePhoto ? (
              <img
                src={profilePhoto}
                alt="Profile"
                className="h-28 w-28 rounded-full border-4 border-white object-cover shadow-lg ring-1 ring-[var(--color-border)]"
              />
            ) : (
              <div className="flex h-28 w-28 items-center justify-center rounded-full border-4 border-white bg-gradient-to-br from-[var(--color-primary)] to-[#6D28D9] text-3xl font-black text-white shadow-lg">
            ...
            <button
              ...
              className="absolute bottom-0 right-0 flex h-9 w-9 items-center justify-center rounded-full border-[3px] border-white bg-[var(--color-primary)] text-white shadow-lg transition hover:scale-105 hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {uploading ? (
            ...
            {isEducator && (
              <div>
                ...
                <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] px-5">
                  ...
                  <InfoRow
                    label="Subjects"
                    value={subjects.length ? subjects.join(", ") : null}
                  />
                </div>
              </div>
            )}
                ...
                <div>
                  ...
                  <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] px-5">
                    ...
                    <InfoRow
                      ...
                      value={
                        strongSubjects.length ? strongSubjects.join(", ") : null
                      }
                    />
                    <InfoRow
                      label="Weak Subjects"
                      value={
                        weakSubjects.length ? weakSubjects.join(", ") : null
                      }
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <FieldInput
                      label="Academic Score"
                      value={latestAcademicScore}
                      onChange={setLatestAcademicScore}
                      maxLength={20}
                      placeholder="e.g. 92%"
                    />
                    <div className="sm:col-span-2">
