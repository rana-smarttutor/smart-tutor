...
import type { ManagedUser, MessageItem, Role, SessionUser, ChatAttachment } from "@/lib/types";
...
export function ChatView({
  ...
  const [validationWarning, setValidationWarning] = useState<ValidationWarning>({ show: false, reasons: [], originalBody: "" });
  ...
  const chatContacts = useMemo(() => {
    ...
    return [];
  }, [role, managedUsers, studentDirectory, studentEducators, assignedFacultyIds]);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(`chat_last_read_${session.id}`, JSON.stringify(lastReadTimestamps));
    } catch {}
      ...
      if (result.hasSensitiveContent) {
        setValidationWarning({
          ...
          reasons: result.reasons.map((r) => {
            ...
            if (r.type === "email") return `Email address detected: ${r.detail}`;
  ...
  async function sendMessage(body: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errData.reasons) {
          setValidationWarning({ show: true, reasons: errData.reasons, originalBody: body });
        }
        return;
      }
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 20 * 1024 * 1024) {
      setFileWarning("File size exceeds 20MB limit. Please choose a smaller file.");
          ...
          {chatContactId && selectedContact ? (
            <>
              ...
              <div className="mx-4 mt-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-2 flex items-center gap-2">
                ...
                <p className="text-[11px] font-medium text-amber-800">
                  Sharing personal contact info or external links is against chat policy and will be blocked.
                </p>
              </div>
              ...
              <div className="border-t border-[var(--color-border)] p-4 bg-[var(--color-panel)]">
                {fileWarning ? (
                  <div className="mb-3 rounded-xl bg-red-50 border border-red-200 px-4 py-2">
                    <p className="text-xs font-semibold text-red-600">{fileWarning}</p>
                  </div>
                ) : null}
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    placeholder={chatBlocked ? "Chat is blocked" : "Type a message..."}
                  ...
                  <button
                    ...
                    disabled={sendingChat || !chatInput.trim() || uploadingFile || chatBlocked}
                  ...
                  >
                    {sendingChat ? (
                      ...
                      <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" aria-hidden="true">
                        <path d="M4.5 4.5L20 12L4.5 19.5L7.2 12L4.5 4.5Z" fill="currentColor" />
                      </svg>
                    )}
                  </button>
                </div>
                <p className="mt-2 text-[10px] text-[var(--color-muted)] text-center">
                  Files up to 20MB. Sharing contact info or external links is against policy.
                </p>
              </div>
            </>
          ) : (
            <div className="hidden lg:flex h-full items-center justify-center">
              <div className="text-center">
                <svg className="h-12 w-12 mx-auto text-[var(--color-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-md rounded-[1.5rem] border border-[var(--color-border)] bg-white p-6 shadow-2xl">
            ...
            <p className="mt-4 text-xs leading-5 text-[var(--color-muted)]">
              Sharing personal contact details or external links is against institute policy. This message has been <strong>blocked</strong>. Please remove any sensitive info before sending.
            </p>
