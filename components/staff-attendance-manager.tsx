...
export function StaffAttendanceManager({
  ...
  const [viewTab, setViewTab] = useState<"admin" | "mine">(
  ...
  const [editMap, setEditMap] = useState<
    Record<
      string,
      { status: StaffAttendanceStatus; checkIn: string; checkOut: string }
    >
  >({});
  ...
  const [regulariseStatus, setRegulariseStatus] =
    useState<StaffAttendanceStatus>("present");
  ...
  const fetchMine = useCallback(async () => {
    if (!userId) return;
    try {
      const res = await fetch(`/api/staff-attendance/my?userId=${userId}`, {
        credentials: "same-origin",
      });
      if (!res.ok) return;
      const data = await res.json();
      ...
      {!embedded ? (
        <div>
          <p className="section-label">
            {role === "admin" ? "Operations" : "My Attendance"}
          </p>
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--color-muted)]">
            {role === "admin"
              ? "Mark staff attendance and manage check-in/check-out times."
              : "Check in or check out for the day and review your attendance history."}
          </p>
        </div>
      ) : null}
          ...
          <div className="flex items-center justify-between gap-3 border-b border-[var(--color-border)] px-5 py-3 flex-wrap">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="rounded-lg bg-[#ECFDF5] px-3 py-1.5 text-xs font-bold text-[#059669] hover:bg-[#D1FAE5]"
              >
                All Present
              </button>
              <button
                ...
                className="rounded-lg bg-[#FEF2F2] px-3 py-1.5 text-xs font-bold text-[#EF4444] hover:bg-[#FEE2E2]"
              >
                All Absent
              </button>
              {role === "admin" && (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setRegulariseStatus(e.target.value as StaffAttendanceStatus)
                  }
                  className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] px-3 py-2 text-sm text-[var(--color-heading)] outline-none focus:border-[var(--color-primary)]"
                >
                  <option value="present">Present</option>
              ...
              <button
                ...
                disabled={
                  submittingRegularise || !regulariseDate || !regulariseReason
                }
